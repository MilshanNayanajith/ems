...
import { ApiResponse, FetchPutHook } from "../types/http_methods";
import api from "./Api";
...
const useFetch_PUT = (): FetchPutHook => {
  ...
  const putData = async (url: string, payload: any): Promise<void> => {
    setIsLoading(true);
    dispatch(setLoading(true));

    try {
      const response: ApiResponse = await api.put(url, payload);
      setData(response?.data);
      setError(null); // Clear previous errors
      dispatch(setAlert({ message: "Data updated successfully", type: "success" }));
    } catch (error: any) {
      if (error?.response) {
